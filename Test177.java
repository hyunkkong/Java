...
/*
 ...
 ○ Serializable 인터페이스
	...
	형식)
	①
	public class 클래스 명 impelements Serializable
	{
		// ...
	}
 ...
 ○ Object Stream
	...
	파일에 저장되어 잇는 객체나 네트워크를 통해 전달된 객체의
	직렬화를 해제하는 기능을 제공한다.
...
public class Test177
{
	public static void main(String[] args) throws Exception
	{
		...
		//테스트(확인)
		//System.out.println(f0.getParentFile().exists());	// 존재하면 true 존재하지 않으면 false
		//--==>> false
		//-- 『test.ser』파일이 만들어질 디렉터리 경로가 구성되어 있지 않다.

		//	 『c:\Java Study』 경로에 『data』 디렉터리 생성 후 다시 확인
		...
		if (f0.exists())
		{
			...
			// fis 파일 입력 스트림으로부터 객체 입력스트림(ois, ObjectInputStream)을 얻어내어
			ObjectInputStream ois = new ObjectInputStream(fis);

			// 객체 입력 스트림(ois, ObjectInputStream)로 부터 읽어들인 객체(Object)를
			// 캐스팅(Hashtable)하여
			...
			while (e.hasMoreElements())
			{
				...
				*/
				...
				//-- Hashtable 자료구조를 대상으로key 를 활용하여 value 를 얻어내는 과정
